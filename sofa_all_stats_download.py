...
def get_all_events(driver, player_id, max_pages=7):
    ...
    for number in range(max_pages):
        ...
        try:
            ...
            events_in_page = data["events"]
            print(f"   → {len(events_in_page)} eventos en página {number}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in (3, 4):
        print("Uso: python3 sofa_all_stats_download.py 'Nombre Jugador' sofascore_id ['Nombre Equipo']")
        sys.exit(1)
    ...
    team_name = normalize_name(sys.argv[3]) if len(sys.argv) >= 4 else None

    base_folder = os.path.join("player_stats", team_name) if team_name else "player_stats"
