...
def scrape_sofascore_json(team_id, player_id):
    driver = start_browser()
    print(f"🌐 Abriendo directamente la URL de la API…")
    API_URL = "https://www.sofascore.com/api/v1/event/"+str(team_id)+"/player/"+str(player_id)+"/statistics"
    driver.get(API_URL)
...
if __name__ == "__main__":
    ...
    # --- 3. Cargar partidos ---
    with open("real_madrid_match_ids.json", "r", encoding="utf-8") as f:
        matches = json.load(f)

    for team, team_id in matches.items():
        FILENAME = "all_stats_madrid_"+str(team)+".json"
        print("Analyzing Real Madrid VS ",team)
        ...
        for name, player_id in players.items():
            ...
            match_stats = scrape_sofascore_json(team_id, player_id)
